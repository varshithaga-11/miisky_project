...
export default function DepartmentDetailsTwo() {
    const { setHeaderStyle, setBreadcrumbTitle } = useLayout();
    const [department, setDepartment] = useState<any>(MOCK_DEPARTMENTS[1] || {});
    ...
    useEffect(() => {
        const fetchDepartment = async () => {
            try {
                const response = await getDepartmentById(2);
                setDepartment(response.data || MOCK_DEPARTMENTS[1]);
            } catch (err) {
                console.warn('Failed to fetch department:', err);
                setDepartment(MOCK_DEPARTMENTS[1]);
            }
        };
                            ...
                            <div className="col-lg-8 col-md-12 col-sm-12 content-side">
                                <div className="service-details-content">
                                    <div className="content-one mb_40">
                                        <figure className="image-box mb_60"><Image src={department.image_url || "/website/assets/images/service/service-6.jpg"} alt={department.name} width={856} height={525} priority /></figure>
