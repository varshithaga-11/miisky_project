...
export default function DepartmentDetailsThree() {
    const { setHeaderStyle, setBreadcrumbTitle } = useLayout();
    const [department, setDepartment] = useState<any>(MOCK_DEPARTMENTS[2] || {});
    ...
    useEffect(() => {
        const fetchDepartment = async () => {
            try {
                const response = await getDepartmentById(3);
                setDepartment(response.data || MOCK_DEPARTMENTS[2]);
            } catch (err) {
                console.warn('Failed to fetch department:', err);
                setDepartment(MOCK_DEPARTMENTS[2]);
            }
        };
