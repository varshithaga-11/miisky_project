  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="max-h-[70vh] overflow-y-auto divide-y divide-gray-100 dark:divide-white/[0.05]">
            {filteredTickets.length === 0 ? (
              <div className="px-4 py-12 text-center text-gray-500">
                <FiMessageSquare className="mx-auto mb-3 text-gray-300" size={32} />
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <div className="font-bold text-gray-900 dark:text-white truncate text-sm mb-0.5 group-hover:text-blue-600 transition-colors">
                        #{t.id} {t.title}
                      </div>
                  ...
                  <div className="mt-2 text-[10px] font-bold uppercase tracking-widest text-gray-400 flex items-center gap-1.5">
        ...
        <div className="lg:col-span-2 rounded-2xl border border-gray-200 dark:border-white/[0.05] bg-white dark:bg-white/[0.03] overflow-hidden flex flex-col">
          <div className="px-4 py-3 border-b border-gray-100 dark:border-white/[0.05]">
            {activeTicket ? (
              ...
              <div className="text-gray-500">Select a ticket to view chat history</div>
            )}
          </div>
          ...
          <div className="h-[55vh] overflow-y-auto px-4 py-4 space-y-3">
            {!activeTicket ? (
              <div className="h-full flex flex-col items-center justify-center text-center p-10 space-y-4">
                ...
                <div>
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white">Active Support Ticket</h3>
                  <p className="text-sm text-gray-500 max-w-xs mx-auto mt-1">Select a ticket from the sidebar to view the conversation history and send updates.</p>
                </div>
                <Button onClick={() => { setIsNewOpen(true); loadCategories(); }} className="rounded-2xl px-8 shadow-lg shadow-blue-500/20">
                   Create New Ticket
                </Button>
              </div>
            ) : messages.length === 0 && attachments.length === 0 ? (
              <div className="text-center text-gray-500 py-10 italic">Awaiting response from support...</div>
            ) : (
                      ...
                      <div className={`max-w-[80%] rounded-2xl px-4 py-3 shadow-sm border border-dashed transition-all ${isMe ? "bg-blue-50 border-blue-200 text-blue-800 rounded-tr-none" : "bg-gray-50 border-gray-200 text-gray-800 rounded-tl-none"
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="text-[10px] opacity-70 truncate">Shared by {isMe ? "you" : formatName(a.uploaded_by_details)}</div>
                          </div>
          ...
          {activeTicket && (
            <div className="px-4 py-3 border-t border-gray-100 dark:border-white/[0.05] space-y-3">
              {pendingFile && (
                <div className="flex items-center gap-2 p-2 bg-blue-50 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-800 rounded-xl max-w-xs transition-all animate-in fade-in slide-in-from-bottom-2">
                  ...
                  <div className="flex-1 min-w-0 pr-2">
                    ...
                    <div className="text-[10px] text-blue-600">Ready to upload</div>
                  </div>
                  <button
              ...
              <div className="flex gap-2 items-end">
                ...
                <Button onClick={handleSendMessage} disabled={uploading || (!messageText.trim() && !pendingFile)} className="h-11 px-5 shadow-lg shadow-blue-500/20 active:scale-95">
                  <FiSend />
                </Button>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-[100000] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-2xl rounded-3xl bg-white dark:bg-gray-900 p-6 border border-gray-200 dark:border-white/[0.05]">
            <div className="flex items-center justify-between mb-4">
              <div>
                <div className="text-xl font-bold text-gray-900 dark:text-white">New Support Ticket</div>
                <div className="text-sm text-gray-500">Provide details about your query or issue.</div>
              </div>
              <button onClick={() => setIsNewOpen(false)} className="w-10 h-10 flex items-center justify-center rounded-2xl bg-gray-100 hover:bg-rose-50 hover:text-rose-600 transition-all text-gray-500">
                <FiX size={20} />
              </button>
            </div>
            ...
            <form onSubmit={handleCreate} className="space-y-4 max-h-[70vh] overflow-y-auto pr-1 no-scrollbar">
              <div className="space-y-2">
                <label className="text-sm font-semibold text-gray-700 dark:text-gray-300 italic flex items-center gap-2">
                   <FiInfo size={14} className="text-blue-500" /> Specify Target Recipient
                </label>
                <div className="grid grid-cols-2 gap-4">
                  {[
                    ...
                    <button
                      ...
                      className={`flex flex-col items-center justify-center p-4 rounded-2xl border-2 transition-all gap-1.5 ${form.target_user_type === target.id
                           ? "border-blue-500 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 shadow-md"
                           : "border-gray-100 dark:border-white/5 hover:border-blue-200"
                        }`}
                    >
                      <span className="text-2xl">{target.icon}</span>
                      <span className="text-[10px] font-black uppercase tracking-tight">{target.label}</span>
                      <span className="text-[9px] opacity-60 text-center">{target.desc}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              {form.target_user_type === "kitchen" && (
                <div className="space-y-1 animate-in fade-in slide-in-from-top-2">
                  <label className="text-sm font-semibold text-gray-700 dark:text-gray-300">Select Micro Kitchen</label>
                  <select
                  ...
                  >
                    <option value="">-- Choose Assigned Kitchen --</option>
                    {kitchens.map(k => (
                      <option key={k.id} value={k.id}>{k.name}</option>
                    ))}
                  </select>
                  {kitchens.length === 0 && (
                    <p className="text-[10px] text-amber-600 font-medium bg-amber-50 dark:bg-amber-900/10 p-2 rounded-lg border border-amber-100 dark:border-amber-900/30">
                      Note: We couldn't find any micro-kitchens currently assigned to you. You may send the request to Admin instead.
                    </p>
                  )}
                </div>
              )}
              ...
              <div className="flex justify-end gap-3 pt-2">
                <Button type="button" variant="outline" onClick={() => setIsNewOpen(false)} className="rounded-xl px-6">
                  Discard
                </Button>
                <Button type="submit" className="rounded-xl px-8 shadow-lg shadow-blue-500/20 active:scale-95 transition-all">
                  Submit Request
                </Button>
              </div>
            </form>
