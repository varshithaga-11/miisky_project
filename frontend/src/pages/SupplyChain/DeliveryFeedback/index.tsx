...
import { FiAlertTriangle, FiHash, FiLoader, FiSearch, FiStar } from "react-icons/fi";
import { DeliveryFeedbackRow, fetchSupplyChainDeliveryFeedbackList } from "./api";

const SupplyChainDeliveryFeedbackPage: React.FC = () => {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        const data = await fetchSupplyChainDeliveryFeedbackList({
          page,
          limit,
  ...
  useEffect(() => {
    ...
    const sentinel = document.getElementById("supplychain-feedback-scroll-sentinel");
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 dark:bg-gray-900/50">
      <PageMeta title="My delivery feedback" description="View feedback on your deliveries" />
      <PageBreadcrumb pageTitle="My Delivery Feedback" />
      ...
      <div className="px-4 md:px-8 pb-20">
        <div className="bg-white dark:bg-gray-800 rounded-[32px] border border-gray-100 dark:border-white/5 p-6 md:p-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h2 className="text-2xl font-black text-gray-900 dark:text-white uppercase tracking-tight">
                Feedback On Your Deliveries
              </h2>
              <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">
                Review ratings and issues submitted for orders and meal deliveries handled by you.
              </p>
            </div>
                ...
                {[
                  ...
                  <button
                    ...
                    onClick={() => {
                      setPage(1);
                      setRows([]);
                      setTargetType(value as "all" | "order" | "user_meal");
                    }}
                    className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${targetType === value ? "bg-indigo-500 text-white" : "text-gray-500 hover:bg-white dark:hover:bg-white/[0.06]"}`}
                  >
                    {label}
                  </button>
                ))}
                ...
                {rows.map((r) => {
                  const reporter =
                    r.reported_by_details
                      ...
                      : "—";
                  const targetLabel = r.order ? `Order #${r.order}` : r.user_meal ? `UserMeal #${r.user_meal}` : "—";
                  const targetMeta = r.order
                    ...
                    : r.user_meal
                      ...
                      : "—";
                  return (
                    <div key={r.id} className="rounded-3xl border border-gray-100 dark:border-white/5 bg-white dark:bg-gray-900/20 p-5 md:p-6">
                      ...
                      {r.feedback_type === "rating" ? (
                        <div className="mt-4 bg-amber-50/50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-500/10 rounded-2xl p-4">
                          <div className="flex items-center gap-1">
                            {[1, 2, 3, 4, 5].map((s) => (
                              <FiStar key={s} size={14} className={s <= (r.rating || 0) ? "text-amber-500 fill-amber-500" : "text-gray-200"} />
                            ))}
                            <span className="ml-2 text-xs font-black text-gray-900 dark:text-white">{r.rating || 0}/5</span>
                          </div>
                        ...
                        <div className="mt-4 bg-rose-50/50 dark:bg-rose-900/10 border border-rose-100 dark:border-rose-500/10 rounded-2xl p-4">
                          <p className="text-[10px] font-black uppercase tracking-widest text-rose-600 mb-1">Issue type</p>
          ...
          <div id="supplychain-feedback-scroll-sentinel" className="h-24 flex items-center justify-center mt-6">
            {fetchingMore && (
            ...
            {!loading && !hasMore && rows.length > 0 && (
              <p className="text-xs text-gray-400 font-bold">
                Loaded all {count} feedback entries across {totalPages} pages.
              </p>
            )}
          </div>
        </div>
...
export default SupplyChainDeliveryFeedbackPage;
