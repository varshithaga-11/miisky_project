...
const LegalPageList: React.FC = () => {
  ...
  const fetchLegalPages = useCallback(async () => {
    setLoading(true);
    try {
      const data = await getLegalPageList(currentPage, pageSize, search);
          ...
          <Table>
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {loading && legalPages.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={6} className="px-5 py-8 text-center text-gray-500 dark:text-gray-400">Loading documentation...</TableCell>
                </TableRow>
              ) : legalPages.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={6} className="px-5 py-8 text-center text-gray-500 dark:text-gray-400">No legal pages found</TableCell>
                </TableRow>
              ) : (
                legalPages.map((page, index) => (
                  <TableRow key={page.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/20 transition-colors">
                    ...
                    <TableCell className="px-5 py-4 text-start">
                         <span className="text-xs font-bold text-gray-600 bg-gray-100 px-2.5 py-1 rounded-md border border-gray-200 uppercase dark:bg-white/5 dark:text-gray-400 dark:border-white/10 uppercase tracking-wider">
                           {page.page_type.replace(/_/g, " ")}
                         </span>
                    </TableCell>
                    <TableCell className="px-5 py-4 text-start font-mono text-xs text-gray-500 dark:text-gray-400">
                        {page.last_updated || "-"}
                    </TableCell>
                    <TableCell className="px-5 py-4 text-start">
                        <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                            page.is_active ? "bg-green-50 text-green-600 dark:bg-green-900/30" : "bg-red-50 text-red-600 dark:bg-red-900/30"
                        }`}>
                    ...
                    <TableCell className="px-5 py-4 text-start text-theme-sm">
                        <div className="flex items-center gap-3">
                            <button className="text-blue-600 hover:text-blue-800 text-lg" onClick={() => setEditingPage(page)}><FiEdit /></button>
                            <button className="text-red-600 hover:text-red-800 text-lg" onClick={() => handleDelete(page.id!)}><FiTrash2 /></button>
                        </div>
                    </TableCell>
                  </TableRow>
                ))
