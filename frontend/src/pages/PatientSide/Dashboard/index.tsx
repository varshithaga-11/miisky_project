      ...
      <header className="mb-12 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
        <div className="space-y-4">
          <div className="flex items-center gap-4">
             <div className="p-4 bg-rose-500 rounded-3xl shadow-xl shadow-rose-500/20 rotate-3 transition-transform hover:rotate-0">
             ...
             <div>
               <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tighter uppercase italic leading-none">
                 My <span className="text-rose-500">Wellness</span>
               </h1>
               <div className="h-1.5 w-16 bg-rose-500 mt-2 rounded-full" />
             </div>
          </div>
          <p className="text-slate-500 dark:text-slate-400 font-medium max-w-xl leading-relaxed text-sm">
            Personal Health Command: Monitor your dietary journey, connect with clinical professionals, and manage your wellness resources.
          </p>
        </div>

        <div className="flex flex-col sm:flex-row items-center gap-6 bg-white dark:bg-white/[0.03] p-3 rounded-[32px] border border-slate-200/60 dark:border-white/5 shadow-sm backdrop-blur-xl">
           <div className="flex items-center gap-4 pl-4">
              {[1, 2, 3].map((i) => (
                <div key={i} className={`size-8 rounded-full flex items-center justify-center transition-all ${
                  i <= progressCount ? 'bg-emerald-500 text-white shadow-lg' : 'bg-slate-100 dark:bg-white/10 text-slate-400'
                }`}>
                  {i <= progressCount ? <CheckCircle size={18} /> : <span className="text-xs font-black">{i}</span>}
                </div>
              ))}
              <div className="pr-6 border-r border-slate-100 dark:border-white/5">
                 <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block leading-none mb-1">Onboarding</span>
                 <span className="text-lg font-black text-slate-900 dark:text-white uppercase leading-none">{progressCount}/3 Steps</span>
              </div>
           </div>
           <button 
             onClick={fetchStats}
             disabled={loading}
             className="p-4 rounded-full bg-slate-100 dark:bg-white/5 text-slate-500 hover:bg-rose-50 hover:text-rose-600 transition-all border border-transparent hover:border-rose-100"
           >
                        ...
                        >
                          <div className="relative h-full overflow-hidden rounded-[54px] border border-gray-200 dark:border-white/[0.05] bg-white dark:bg-white/[0.02] p-10 shadow-sm group-hover:shadow-2xl group-hover:border-rose-500/40 transition-all backdrop-blur-xl">
                            ...
                            <div className="flex justify-between items-start mb-10 relative z-10">
                              <div className={`p-5 rounded-[28px] shadow-sm transition-all duration-700 group-hover:scale-110 group-hover:rotate-6 ${
