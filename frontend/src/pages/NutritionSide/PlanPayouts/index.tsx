...
import { 
  fetchNutritionistPayoutPatients, 
...
export default function NutritionPlanPayoutsPage() {
  ...
  const loadPatients = useCallback(async () => {
    ...
    try {
      const res = await fetchNutritionistPayoutPatients(patientPage, 12);
  ...
  return (
    <>
      ...
      <div className="max-w-6xl mx-auto space-y-8 pb-20">
        <div className="bg-white dark:bg-gray-900 rounded-3xl p-8 border border-gray-100 dark:border-gray-800 shadow-sm border-b-4 border-b-brand-500">
           <h1 className="text-3xl font-black text-gray-900 dark:text-white flex items-center gap-3">
             <span className="bg-brand-500 text-white p-2 rounded-2xl shadow-lg shadow-brand-200 dark:shadow-none">
           ...
           <p className="text-gray-500 dark:text-gray-400 mt-4 max-w-2xl leading-relaxed">
             Track your revenue share for patient diet plans. Payouts are grouped by patient. Expand a patient to see their plans and view detailed payment history using the <span className="text-brand-600 font-bold">eye icon</span>.
           </p>
        </div>
            ...
            <>
              <div className="p-6 space-y-3">
                {patientData.results.map((patient) => {
                  ...
                  return (
                    <div key={patient.id} className={`group border transition-all duration-300 rounded-2xl overflow-hidden ${isExpanded ? "border-brand-200 shadow-lg ring-1 ring-brand-100" : "border-gray-100 hover:border-brand-300 hover:shadow-md"}`}>
                      <div className={`p-5 flex items-center justify-between cursor-pointer ${isExpanded ? "bg-brand-50/30 dark:bg-brand-900/10" : "bg-white dark:bg-gray-900"}`} onClick={toggleExpand}>
                        <div className="flex items-center gap-4">
                           <div className="w-12 h-12 bg-gray-100 dark:bg-gray-800 rounded-xl flex items-center justify-center text-brand-600 font-black text-lg">
                        ...
                        <div className="flex items-center gap-6">
                           ...
                           <button className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${isExpanded ? "bg-brand-600 text-white rotate-180" : "bg-gray-50 text-gray-400 group-hover:bg-brand-100 group-hover:text-brand-600"}`}>
                      ...
                      {isExpanded && (
                        <div className="bg-gray-50/40 dark:bg-gray-900/40 border-t border-brand-100 p-4 space-y-4">
                              ...
                              <div className="p-4 flex flex-wrap items-center justify-between gap-4">
                                ...
                                <div className="flex items-center gap-8 bg-gray-50/50 dark:bg-gray-900/50 px-6 py-2 rounded-2xl border border-gray-100/50 dark:border-gray-700/50">
                                   ...
                                   <div className="flex flex-col">
                                     ...
                                     <span className="text-sm font-bold text-brand-600">₹{parseFloat(t.paid_amount).toFixed(2)}</span>
                                   </div>
                                   <div className="flex flex-col">
                                          ...
                                          <tr key={tx.id} className="text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                            ...
                                            <td className="py-2 px-3 text-right">
                                              {tx.payment_screenshot_url ? (
                                                <a href={tx.payment_screenshot_url} target="_blank" rel="noreferrer" className="w-6 h-6 inline-flex items-center justify-center bg-brand-50 text-brand-600 rounded-md hover:bg-brand-600 hover:text-white transition-colors">
                ...
                <div className="p-6 border-t border-gray-100 dark:border-gray-800 flex justify-center gap-2 bg-gray-50/30 dark:bg-gray-900/30">
                  {Array.from({ length: patientData.totalPages }, (_, i) => i + 1).map((p) => (
                    <button
                      ...
                      className={`w-10 h-10 rounded-xl font-bold transition-all ${patientPage === p ? "bg-brand-600 text-white shadow-lg shadow-brand-200" : "bg-white text-gray-400 hover:bg-brand-50 hover:text-brand-600 border border-gray-100"}`}
                    >
                      {p}
                    </button>
                  ))}
