      ...
      <div className="mb-12 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium max-w-xl leading-relaxed text-sm">
            Clinical Command Center: Optimized for patient care tracking, nutritional planning, and professional collaboration.
          </p>
        </div>
        ...
        <div className="flex items-center gap-6 bg-white dark:bg-white/[0.03] p-3 rounded-[32px] border border-slate-200/60 dark:border-white/5 shadow-sm backdrop-blur-xl">
           <div className="px-8 py-3 rounded-[24px] bg-slate-900 dark:bg-blue-600 text-white shadow-xl shadow-blue-500/10">
              <span className="text-[10px] font-black uppercase tracking-widest block opacity-70 mb-0.5">Clinical Load</span>
              <span className="text-2xl font-black tabular-nums tracking-tighter">{loading ? "—" : totalInteractions.toLocaleString()}</span>
           </div>
           <button 
             onClick={fetchStats}
             disabled={loading}
             className="p-4 rounded-full bg-slate-100 dark:bg-white/5 text-slate-500 hover:bg-blue-50 hover:text-blue-600 transition-all"
           >
                ...
                <section key={category} className="space-y-10">
                  <div className="flex items-center gap-4 ml-2">
                    <div className={`h-10 w-2 rounded-full ${
                      category === 'Patient Care' ? 'bg-blue-600' : 
