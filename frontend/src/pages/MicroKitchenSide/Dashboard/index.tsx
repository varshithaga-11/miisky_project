      ...
      <header className="mb-12 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
        <div>
          <div className="flex items-center gap-4 mb-4">
             <div className="p-4 bg-amber-500 rounded-3xl shadow-xl shadow-amber-500/20 rotate-3 transition-transform hover:rotate-0">
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium max-w-xl leading-relaxed text-sm">
            Operational Hub: Real-time production queue, inventory monitoring, and patient logistics control. 
          </p>
        </div>
        ...
        <div className="flex items-center gap-6 bg-white dark:bg-white/[0.03] p-3 rounded-[32px] border border-slate-200/60 dark:border-white/5 shadow-sm backdrop-blur-xl">
           <div className="px-8 py-3 rounded-[24px] bg-slate-900 dark:bg-amber-500 text-white shadow-xl shadow-amber-500/10">
              <span className="text-[10px] font-black uppercase tracking-widest block opacity-70 mb-0.5">Production Load</span>
              <span className="text-2xl font-black tabular-nums tracking-tighter">{loading ? "—" : totalLoad.toLocaleString()}</span>
           </div>
           <button 
             onClick={fetchStats}
             disabled={loading}
             className="p-4 rounded-full bg-slate-100 dark:bg-white/5 text-slate-500 hover:bg-amber-50 hover:text-amber-600 transition-all"
           >
             ...
             <div className="rounded-[48px] border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-10 shadow-sm relative overflow-hidden group">
                ...
                <div className="flex items-center justify-between mb-10 relative z-10">
                   <div className="flex items-center gap-4">
                      ...
                      <h2 className="text-2xl font-black text-gray-900 dark:text-white tracking-tighter uppercase italic">Inventory Pulse</h2>
                   </div>
                </div>
                <div className="grid grid-cols-2 gap-6 relative z-10">
                   {[
                      { label: "Available Stock", value: stats?.foodsAvailable ?? 0, color: "text-emerald-500", key: "avail" },
                      { label: "Out of Stock", value: stats?.foodsOutOfStock ?? 0, color: "text-rose-500", key: "out" },
                   ].map(stat => (
                      <div key={stat.key} className="p-10 rounded-[44px] bg-slate-50 dark:bg-gray-800/50 border border-transparent transition-all hover:border-emerald-200 hover:bg-emerald-50/30">
                         <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest block mb-3">{stat.label}</span>
                         <span className={`text-5xl font-black tabular-nums tracking-tighter ${stat.color}`}>{stat.value}</span>
                      </div>
                   ))}
