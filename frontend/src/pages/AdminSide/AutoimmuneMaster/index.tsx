...
import { getAutoimmuneMasterList, deleteAutoimmuneMaster, AutoimmuneMaster } from "./api";
import AddAutoimmuneMaster from "./AddAutoimmuneMaster";
import EditAutoimmuneMaster from "./EditAutoimmuneMaster";
...
const AutoimmuneMasterPage: React.FC = () => {
  ...
  const [sortField, setSortField] = useState<keyof AutoimmuneMaster | null>("sort_order");
  ...
  const fetchRows = async () => {
    setLoading(true);
    try {
      const r = await getAutoimmuneMasterList(currentPage, pageSize, searchTerm);
      setRows(r.results);
  ...
  const handleDelete = async (id: number) => {
    try {
      const res = await checkMasterDependency("autoimmune", id);
      if (res.detail !== "none") {
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteMasterRecord("autoimmune", recordToDelete);
  ...
  const handleSort = (field: keyof AutoimmuneMaster) => {
  ...
  return (
    <>
      ...
      {isAddOpen && <AddAutoimmuneMaster onClose={() => setIsAddOpen(false)} onAdd={() => void fetchRows()} />}
...
export default AutoimmuneMasterPage;
