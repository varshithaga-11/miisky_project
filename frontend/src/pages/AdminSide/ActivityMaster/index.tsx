...
import { getActivityMasterList, deleteActivityMaster, ActivityMaster } from "./api";
import AddActivityMaster from "./AddActivityMaster";
import EditActivityMaster from "./EditActivityMaster";
...
const ActivityMasterPage: React.FC = () => {
  const [rows, setRows] = useState<ActivityMaster[]>([]);
  ...
  const [sortField, setSortField] = useState<keyof ActivityMaster | null>("sort_order");
  ...
  const fetchRows = async () => {
    setLoading(true);
    try {
      const r = await getActivityMasterList(currentPage, pageSize, searchTerm);
      setRows(r.results);
  ...
  const handleDelete = async (id: number) => {
    try {
      const res = await checkMasterDependency("activity", id);
      if (res.detail !== "none") {
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteMasterRecord("activity", recordToDelete);
  ...
  const handleSort = (field: keyof ActivityMaster) => {
  ...
  return (
    <>
      <PageMeta title="Physical activities" description="Physical activity catalog" />
      <PageBreadcrumb pageTitle="Physical activities" />
      ...
      {isAddOpen && <AddActivityMaster onClose={() => setIsAddOpen(false)} onAdd={() => void fetchRows()} />}
      {isEditOpen && editId !== null && (
        <EditActivityMaster recordId={editId} isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} onUpdated={() => void fetchRows()} />
      )}
      ...
      <ConfirmationModal
        ...
        message="Are you sure you want to remove this activity from the master list? Linked user records may lose the reference."
