...
const adminMenus: DashboardMenuItem[] = [
  ...
  { label: "Verification", path: "/admin/patient-payment-verification", icon: ShieldCheck, statKey: null, color: "emerald", category: "Care" },
  ...
  { label: "Meal Types", path: "/admin/meal-type", icon: Layers, statKey: null, color: "orange", category: "Food" },
  { label: "Cuisine Types", path: "/admin/cuisine-type", icon: Layers, statKey: null, color: "orange", category: "Food" },
  { label: "Packaging", path: "/admin/packaging-material", icon: Package, statKey: null, color: "slate", category: "Food" },
  { label: "Recipes", path: "/admin/recipe-creator", icon: ClipboardList, statKey: null, color: "teal", category: "Food" },

  // Food Composition
  { label: "Food Groups", path: "/admin/food-group", icon: Layers, statKey: null, color: "sky", category: "Composition" },
  { label: "Food Names", path: "/admin/food-name", icon: FileText, statKey: null, color: "sky", category: "Composition" },
  { label: "Nutrients", path: "/admin/food-proximate", icon: Activity, statKey: null, color: "sky", category: "Composition" },
  ...
  { label: "Diet Plans", path: "/admin/diet-plan", icon: ClipboardList, statKey: null, color: "indigo", category: "System" },
...
export default function AdminSideDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#0B0F1A] p-6 lg:p-10 animate-in fade-in duration-700">
                    ...
                    >
                      <div className="relative h-full overflow-hidden rounded-[40px] border border-slate-200/80 dark:border-white/[0.05] bg-white dark:bg-white/[0.02] p-8 shadow-sm group-hover:shadow-2xl group-hover:border-blue-500/40 transition-all backdrop-blur-md">
                        ...
                        <div className="flex justify-between items-start mb-8 relative z-10">
                          <div className={`p-4 rounded-[22px] shadow-sm transition-all duration-700 group-hover:scale-110 group-hover:rotate-6 ${
      ...
      <footer className="mt-20 grid grid-cols-1 md:grid-cols-2 gap-8 pt-10 border-t border-slate-200 dark:border-white/5">
         <div className="p-8 rounded-[40px] bg-gradient-to-br from-slate-900 to-indigo-950 text-white relative overflow-hidden group shadow-2xl">
            <div className="absolute right-0 top-0 size-64 bg-blue-500/10 blur-[80px] rounded-full scale-150 group-hover:scale-110 transition-transform duration-700"></div>
            <div className="relative z-10 space-y-4">
               <div className="p-3 bg-white/10 rounded-2xl w-fit">
                  <ShieldCheck size={28} />
               </div>
               <h3 className="text-3xl font-black tracking-tighter uppercase italic leading-none">Security & Compliance</h3>
               <p className="text-slate-300 text-sm font-medium leading-relaxed max-w-sm">
                 All data displayed is processed via secured SSL/TLS endpoints. Access is strictly audited under the platform's RBAC protocols.
               </p>
               <button className="mt-4 px-6 py-2.5 bg-white text-slate-900 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-blue-400 hover:text-white transition-all shadow-lg">
                  Audit Logs
               </button>
            </div>
         </div>

         <div className="p-8 rounded-[40px] bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 relative overflow-hidden group shadow-xl">
            <div className="relative z-10 grid grid-cols-2 gap-8 h-full">
               <div className="space-y-4">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">System Nodes</span>
                  <div className="flex flex-col gap-3">
                     {[
                        { l: "Cloud API", s: "Operational", c: "emerald" },
                        { l: "ML Engine", s: "Processing", c: "blue" },
                     ].map((n, i) => (
                        <div key={i} className="flex items-center gap-3">
                           <div className={`size-2 rounded-full bg-${n.c}-500 shadow-[0_0_8px_rgba(0,0,0,0.1)]`}></div>
                           <div>
                              <div className="text-xs font-bold text-slate-800 dark:text-white">{n.l}</div>
                              <div className="text-[9px] font-black text-slate-400 uppercase">{n.s}</div>
                           </div>
                        </div>
                     ))}
                  </div>
               ...
               <div className="space-y-4">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Quick Options</span>
                  <div className="flex flex-col gap-2">
                     <button className="flex items-center gap-2 p-2 rounded-xl bg-slate-50 dark:bg-white/5 text-[10px] font-black uppercase tracking-tight text-slate-600 dark:text-slate-400 hover:bg-blue-50 transition-colors">
