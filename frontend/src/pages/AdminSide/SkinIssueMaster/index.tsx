...
const SkinIssueMasterPage: React.FC = () => {
  ...
  const handleDelete = async (id: number) => {
    try {
      const res = await checkMasterDependency("skin_issue", id);
      if (res.detail !== "none") {
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteMasterRecord("skin_issue", recordToDelete);
      toast.success("Skin issue deleted successfully!");
      setRecordToDelete(null);
      void fetchRows();
    } catch {
      toast.error("Failed to delete skin issue.");
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  return (
    <>
      ...
      <ConfirmationModal
        ...
        title="Delete Record?"
        message="Are you sure you want to remove this skin issue from the master list?"
