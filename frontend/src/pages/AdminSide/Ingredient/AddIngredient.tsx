...
import { createIngredient } from "./ingredientapi";
...
const AddIngredient: React.FC<{ onClose: () => void; onAdd: () => void }> = ({ onClose, onAdd }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(isAdmin ? "Ingredient created!" : "Sent for approval");
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg w-full max-w-sm relative shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <Input value={name} onChange={e => setName(e.target.value)} required placeholder="e.g. Rice" />
          </div>
