...
export interface DeficiencyMaster {
  id?: number;
  name: string;
}
...
const basePath = "api/deficiency-master/";

export const createDeficiencyMaster = async (data: DeficiencyMaster) => {
  return (await axios.post(createApiUrl(basePath), data, { headers: await getAuthHeaders() })).data;
};

export const getDeficiencyMasterList = async (
  page: number = 1,
  limit: number | "all" = 10,
  search?: string
): Promise<PaginatedResponses<DeficiencyMaster>> => {
  ...
  if (isAll) {
    const rows = response.data as DeficiencyMaster[];
    return { count: rows.length, next: null, previous: null, current_page: 1, total_pages: 1, results: rows };
  }
  return response.data as PaginatedResponses<DeficiencyMaster>;
};

export const getDeficiencyMasterById = async (id: number) => {
  return (await axios.get(createApiUrl(`${basePath}${id}/`), { headers: await getAuthHeaders() })).data;
};

export const updateDeficiencyMaster = async (id: number, data: Partial<DeficiencyMaster>) => {
  return (await axios.put(createApiUrl(`${basePath}${id}/`), data, { headers: await getAuthHeaders() })).data;
};

export const deleteDeficiencyMaster = async (id: number) => {
