...
import { getDeficiencyMasterList, deleteDeficiencyMaster, DeficiencyMaster } from "./api";
import AddDeficiencyMaster from "./AddDeficiencyMaster";
import EditDeficiencyMaster from "./EditDeficiencyMaster";
...
const DeficiencyMasterPage: React.FC = () => {
  const [rows, setRows] = useState<DeficiencyMaster[]>([]);
  ...
  const [sortField, setSortField] = useState<keyof DeficiencyMaster | null>("sort_order");
  ...
  const fetchRows = async () => {
    setLoading(true);
    try {
      const r = await getDeficiencyMasterList(currentPage, pageSize, searchTerm);
      setRows(r.results);
  ...
  const handleDelete = async (id: number) => {
    try {
      const res = await checkMasterDependency("deficiency", id);
      if (res.detail !== "none") {
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteMasterRecord("deficiency", recordToDelete);
      toast.success("Deficiency record deleted successfully!");
      setRecordToDelete(null);
      void fetchRows();
    } catch {
      toast.error("Failed to delete deficiency record.");
    } finally {
      setIsDeleting(false);
    }
  };

  const handleSort = (field: keyof DeficiencyMaster) => {
  ...
  return (
    <>
      <PageMeta title="Deficiencies" description="Deficiency catalog" />
      <PageBreadcrumb pageTitle="Deficiencies" />
      ...
      {isAddOpen && <AddDeficiencyMaster onClose={() => setIsAddOpen(false)} onAdd={() => void fetchRows()} />}
      {isEditOpen && editId !== null && (
        <EditDeficiencyMaster recordId={editId} isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} onUpdated={() => void fetchRows()} />
      )}
      ...
      <ConfirmationModal
        ...
        title="Delete Record?"
        message="Are you sure you want to remove this deficiency from the master list?"
...
export default DeficiencyMasterPage;
