...
import { getHabitMasterById, updateHabitMaster } from "./api";
...
const EditHabitMaster: React.FC<Props> = ({ recordId, isOpen, onClose, onUpdated }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await updateHabitMaster(recordId, { name: name.trim(), sort_order: Number(sortOrder) || 0 });
  ...
  return (
    <div className="fixed inset-0 bg-white/50 backdrop-blur-sm flex items-center justify-center z-50">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 w-full max-w-md relative mt-24">
        ...
        <h2 className="text-2xl font-bold mb-4 text-gray-900 dark:text-white">Edit lifestyle habit</h2>
        {loading ? (
...
export default EditHabitMaster;
