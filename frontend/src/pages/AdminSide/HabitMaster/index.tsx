...
import { getHabitMasterList, deleteHabitMaster, HabitMaster } from "./api";
import AddHabitMaster from "./AddHabitMaster";
import EditHabitMaster from "./EditHabitMaster";
...
const HabitMasterPage: React.FC = () => {
  const [rows, setRows] = useState<HabitMaster[]>([]);
  ...
  const [sortField, setSortField] = useState<keyof HabitMaster | null>("sort_order");
  ...
  const fetchRows = async () => {
    setLoading(true);
    try {
      const r = await getHabitMasterList(currentPage, pageSize, searchTerm);
      setRows(r.results);
  ...
  const handleDelete = async (id: number) => {
    try {
      const res = await checkMasterDependency("habit", id);
      if (res.detail !== "none") {
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteMasterRecord("habit", recordToDelete);
  ...
  const handleSort = (field: keyof HabitMaster) => {
  ...
  return (
    <>
      <PageMeta title="Lifestyle habits" description="Lifestyle habit catalog" />
      <PageBreadcrumb pageTitle="Lifestyle habits" />
      ...
      {isAddOpen && <AddHabitMaster onClose={() => setIsAddOpen(false)} onAdd={() => void fetchRows()} />}
      {isEditOpen && editId !== null && (
        <EditHabitMaster recordId={editId} isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} onUpdated={() => void fetchRows()} />
      )}
      ...
      <ConfirmationModal
        ...
        title="Delete habit?"
        message="Are you sure you want to remove this habit from the master list? Linked user records may lose the reference."
...
export default HabitMasterPage;
